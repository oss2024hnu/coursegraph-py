...
## issue 등록시 참고사항
- 어떤 이슈를 만들고자 할때 자신만이 알수 있게 적는 것이 아니라 처음 코드를 보는 사람 , 이슈를 보는 사람도 쉽게 이해할 수 있고 참여할 수 있게 자세하게 설명해주면 좋을것 같습니다. 
- ex : 기존의 코드는 이러한 코드이나 이는 어떤 불편함을 제공하므로 다음 코드와 같이 바꾸면 좋을 것 같습니다.

## pr 등록시 참고사항
- 이슈에 기반한 pr을 등록하고자 할때 처음 참여하는 사람과 , 코드를 보는 사람이 자세하게 이해를 할 수 있도록 설명해주면 좋을것 같습니다.
- 어떤 이슈를 기반으로 pr을 등록했는지를 언급하여 이슈와 pr을 동시에 이해할 수 있게 하는 등의 방법을 사용하면 될것 같습니다. (이슈와 pr을 연결하는 방법은 참고사항을 참고해주세요.) 
- ex : #165의 이슈를 기반으로 이러한 코드로 바꾸었습니다. 코드를 실행할 때는 어떠한 방법으로 코드를 실행하면 될것 같습니다. 
